...
async function extractArchive(archiveFileName, cwd) {
  try {
    runCommand('tar', ['-xf', archiveFileName], cwd);
  } catch (tarError) {
    if (process.platform === 'win32') {
      // Some Windows images expose bsdtar instead of tar.
      runCommand('bsdtar', ['-xf', archiveFileName], cwd);
      return;
    }
    throw tarError;
  }
...
async function fetchSparseRepo(repo, ref, paths, checkoutDir) {
  ...
  runCommand('git', ['init', gitCheckoutDir], ROOT);
  runCommand('git', ['-C', gitCheckoutDir, 'remote', 'add', 'origin', remote], ROOT);
  runCommand('git', ['-C', gitCheckoutDir, 'fetch', '--depth', '1', 'origin', ref], ROOT);
  ...
  const commit = String(runCommand('git', ['-C', gitCheckoutDir, 'rev-parse', 'FETCH_HEAD'], ROOT, true)).trim();
  return commit;
}
