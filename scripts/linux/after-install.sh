...
# Post-installation script for ItemClaw on Linux
...
# Create symbolic link for ItemClaw app binary
if [ -x /opt/ItemClaw/clawx ]; then
    ln -sf /opt/ItemClaw/clawx /usr/local/bin/clawx 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/ItemClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/ItemClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/ItemClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/ItemClaw/resources/apparmor-profile'
...
echo "ItemClaw has been installed successfully."
