; ItemClaw Custom NSIS Installer/Uninstaller Script
;
...
!macro customUnInstall
  ...
  MessageBox MB_YESNO|MB_ICONQUESTION \
    "Do you want to completely remove all ItemClaw user data?$\r$\n$\r$\nThis will delete:$\r$\n  • .openclaw folder (configuration & skills)$\r$\n  • AppData\Local\itemclaw (local app data)$\r$\n  • AppData\Roaming\itemclaw (roaming app data)$\r$\n$\r$\nSelect 'No' to keep your data for future reinstallation." \
    /SD IDNO IDYES _cu_removeData IDNO _cu_skipRemove

  _cu_removeData:
    ; Kill any lingering ItemClaw processes to release file locks on electron-store
    ...
    RMDir /r "$LOCALAPPDATA\itemclaw"
    RMDir /r "$APPDATA\itemclaw"
    ...
    ; Check AppData\Local\itemclaw
    IfFileExists "$LOCALAPPDATA\itemclaw\*.*" 0 _cu_localDone
      Sleep 3000
      RMDir /r "$LOCALAPPDATA\itemclaw"
      IfFileExists "$LOCALAPPDATA\itemclaw\*.*" 0 _cu_localDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$LOCALAPPDATA\itemclaw"'
        Pop $0
        Pop $1
    _cu_localDone:

    ; Check AppData\Roaming\itemclaw
    IfFileExists "$APPDATA\itemclaw\*.*" 0 _cu_roamingDone
      Sleep 3000
      RMDir /r "$APPDATA\itemclaw"
      IfFileExists "$APPDATA\itemclaw\*.*" 0 _cu_roamingDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$APPDATA\itemclaw"'
        Pop $0
        Pop $1
    _cu_roamingDone:
    ...
    IfFileExists "$PROFILE\.openclaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $PROFILE\.openclaw"
    IfFileExists "$LOCALAPPDATA\itemclaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $LOCALAPPDATA\itemclaw"
    IfFileExists "$APPDATA\itemclaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $APPDATA\itemclaw"
  ...
  _cu_enumLoop:
    ...
    RMDir /r "$R2\.openclaw"
    RMDir /r "$R2\AppData\Local\itemclaw"
    RMDir /r "$R2\AppData\Roaming\itemclaw"
