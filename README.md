...
**Item** bridges the gap between powerful AI agents and everyday users. Built on top of [OpenClaw](https://github.com/OpenClaw), it transforms command-line AI orchestration into an accessible, beautiful desktop experience—no terminal required.

Whether you're automating workflows, managing AI-powered channels, or scheduling intelligent tasks, Item provides the interface you need to harness AI agents effectively.

Item comes pre-configured with best-practice model providers and natively supports Windows as well as multi-language settings. Of course, you can also fine-tune advanced configurations via **Settings → Advanced → Developer Mode**.
...
Building AI agents shouldn't require mastering the command line. Item was designed with a simple philosophy: **powerful technology deserves an interface that respects your time.**
...
Item is built directly upon the official **OpenClaw** core. Instead of requiring a separate installation, we embed the runtime within the application to provide a seamless "battery-included" experience.
...
### Adaptive Theming
Light mode, dark mode, or system-synchronized themes. Item adapts to your preferences automatically.
...
Download the latest release for your platform from the [Releases](https://github.com/pablogutierrezit/Itemclawx/releases) page.

#### Build from Source

```bash
# Clone the repository
git clone https://github.com/pablogutierrezit/Itemclawx.git
cd Itemclawx
...
When you launch Item for the first time, the **Setup Wizard** will guide you through:
...
> Note for Moonshot (Kimi): Item keeps Kimi web search enabled by default.  
> When Moonshot is configured, Item also syncs Kimi web search to the China endpoint (`https://api.moonshot.cn/v1`) in OpenClaw config.

### Proxy Settings

Item includes built-in proxy settings for environments where Electron, the OpenClaw Gateway, or channels such as Telegram need to reach the internet through a local proxy client.
...
- If advanced proxy fields are left empty, Item falls back to `Proxy Server`.
- Saving proxy settings reapplies Electron networking immediately and restarts the Gateway automatically.
- Item also syncs the proxy to OpenClaw's Telegram channel config when Telegram is enabled.

---

## Architecture

Item employs a **dual-process architecture** with a unified host API layer. The renderer talks to a single client abstraction, while Electron Main owns protocol selection and process lifecycle:
...
We welcome contributions from the community! Whether it's bug fixes, new features, documentation improvements, or translations—every contribution helps make Item better.
...
Item is built on the shoulders of excellent open-source projects:
...
Item is released under the [MIT License](LICENSE). You're free to use, modify, and distribute this software.

---

<p align="center">
  <sub>Built with care by the Item Team</sub>
</p>
