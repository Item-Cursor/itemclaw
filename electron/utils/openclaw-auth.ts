...
/**
 * Write the ItemClaw gateway token into ~/.openclaw/openclaw.json.
 */
export async function syncGatewayTokenToConfig(token: string): Promise<void> {
  ...
  // Packaged ItemClaw loads the renderer from file://, so the gateway must allow
 ...
 * OpenClaw/ItemClaw versions or external tools.
 *
...
export async function sanitizeOpenClawConfig(): Promise<void> {
  ...
  // When ItemClaw-managed moonshot provider exists, prefer centralized key
