...
import { JiraCredentialDialog } from '@/components/skills/JiraCredentialDialog';
...
const SPECIAL_JIRA_SKILLS = [
  {
    id: 'di-jira',
    name: 'DI Jira',
    description: 'Jira operations specialist for DTS workflow. Configure JIRA_USER and JIRA_PASS to enable API actions.',
  },
  {
    id: 'di-jira_English',
    name: 'DI Jira',
...
const JIRA_SKILL_IDS = SPECIAL_JIRA_SKILLS.map((s) => s.id);
...
export function Skills() {
  ...
  const [jiraConfigOpenSkillId, setJiraConfigOpenSkillId] = useState<string | null>(null);
  ...
  return (
    <div className="flex flex-col -m-6 dark:bg-background h-[calc(100vh-2.5rem)] overflow-hidden">
          ...
          <div className="flex flex-col gap-1">
            {filteredSkills.length === 0 ? (
              ...
              <>
                ...
                {visibleSpecialJiraSkills.map((skill) => {
                  const env = (skill.config?.env as Record<string, unknown> | undefined) || {};
                  const isConfigured = Boolean(env.JIRA_USER) && Boolean(env.JIRA_PASS);
                  return (
                    <div
                      key={skill.id}
                      className="group flex flex-row items-center justify-between py-3.5 px-3 rounded-xl hover:bg-black/5 dark:hover:bg-white/5 transition-colors cursor-pointer border-b border-black/5 dark:border-white/5"
                      onClick={() => setJiraConfigOpenSkillId(skill.id)}
                    >
                      <div className="flex items-start gap-4 flex-1 overflow-hidden pr-4">
                        <div className="h-10 w-10 shrink-0 flex items-center justify-center bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-xl overflow-hidden text-lg">
                          🎯
                        </div>
                        <div className="flex flex-col overflow-hidden">
                          <div className="flex items-center gap-2 mb-1">
                ...
                {genericFilteredSkills.map((skill) => (
                  <div
                    key={skill.id}
      ...
      <JiraCredentialDialog
        skill={(() => {
          if (!jiraConfigOpenSkillId) return null;
          const resolvedSkill = safeSkills.find((s) => s.id === jiraConfigOpenSkillId);
          if (resolvedSkill) return resolvedSkill;
          const fallback = SPECIAL_JIRA_SKILLS.find((s) => s.id === jiraConfigOpenSkillId);
          if (!fallback) return null;
          return {
            id: fallback.id,
            name: fallback.name,
        ...
        isOpen={!!jiraConfigOpenSkillId}
        onClose={() => setJiraConfigOpenSkillId(null)}
        onToggle={(enabled) => {
          if (!jiraConfigOpenSkillId) return;
          handleToggle(jiraConfigOpenSkillId, enabled);
        }}
      />
    </div>
  );
